...
class YOLODetector(BaseDetector):
    ...
    def detect(self, frame):
        """Pure detection: returns [ [x1, y1, x2, y2, conf], ... ]"""
        results = self.model_.predict(source=frame, conf=self.threshold_, verbose=False)

        if not results or len(results[0].boxes) == 0:
            return []
        
        # Extract raw coordinates and scores
        boxes = results[0].boxes.xyxy.cpu().numpy()
        confs = results[0].boxes.conf.cpu().numpy()
        
        # Combine into a single Nx5 array (standard for trackers)
        return [[*box, conf] for box, conf in zip(boxes, confs)]
